...
const USE_MOCK_DATA = false; // Mock veri modunu kapattık, artık gerçek backend API'ye bağlanır

// Mock veri generator
function generateMockTelemetry(limit: number): TelemetryReading[] {
    const segments = ['S1', 'S2', 'S3', 'S4', 'S5', 'S6'];
    const readings: TelemetryReading[] = [];
    const channelNames = ['ray_temperature', 'ray_vibration_x', 'rail_slope', 'train_temperature', 'train_speed', 'train_vibration_x'];
    ...
    for (let i = 0; i < limit; i++) {
        ...
        readings.push({
            ...
            recordedAt,
            unit: channelName.includes('temperature') ? '°C' : channelName.includes('vibration') ? 'Hz' : channelName.includes('speed') ? 'km/h' : '°'
        });
    }
...
export const telemetryService = {
    ...
    async getLatestTelemetry(limit: number = 100): Promise<TelemetryReading[]> {
        ...
        try {
            // Spring Data Pageable expects 'size' instead of 'limit'
            const response = await fetch(`${API_URL}/telemetry?size=${limit}`);
            if (!response.ok) throw new Error("Network response was not ok");
            const data = await response.json();
            // Handle Spring Data Page object structure
